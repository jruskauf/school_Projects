...
code_t purge(Queue **myQueue)
{
    ...
    //if the queue is invalid or null, report an error.
    if(myQueue){
	if((*myQueue) != NULL){
	    //if the queue is empty, report a successful operation, otherwise
	    //we remove the list, and then null the front and back of the queue
	    //after which we report a success.
