...
code_t enqueue(Queue **myQueue, Node *newNode)
{
    ...
    //if the queue is invalid, null, or if newnode is null, or if list qty
    //is greater/equal to the buffer, fall through and report an error.
    //otherwise, continue with normal operations.
    if(myQueue != NULL){
	if((*myQueue) != NULL){
	    if(newNode != NULL){
		if((*myQueue) -> data -> qty < (*myQueue) -> buffer || (*myQueue) -> buffer == 0){ 
		    //insert the new node to the front of the queue, tracking the front and back
		    //of the queue properly.  then report a successful operation.
