...
code_t rmqueue(Queue **curQueue)
{
    ...
    //if the current queue is invalid or null, report an error, otherwise if
    //the queue is populated, purge and null it.  If it's already empty, then
    //null the queue as we don't need to do anything else.
    if(curQueue != NULL){
	if((*curQueue) != NULL){
