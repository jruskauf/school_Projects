...
int main()
{
	...
	for(i = 0; i < 4; i++)
	{
		...
		ntmp     = mknode(data[i]);
		fprintf(stdout, " you have: "); 
		ltmp     = append(ltmp, ltmp -> lead, ntmp);
		displayf(ltmp, 0);
		fprintf(stdout, "should be: %s\n\n", output[num++]); 
		fflush(stdout);
	...
	fprintf(stdout, "Test %d: Appending %d after second to last node ...\n", testno++, data[i]);
	fprintf(stdout, " you have: ");
	...
	fprintf(stdout, "Test %d: Appending %d after third to last node ...\n", testno++, data[i]);
	fprintf(stdout, " you have: ");
	...
	fprintf(stdout, "Test %d: Appending %d after fifth to last node ...\n", testno++, data[i]);
	fprintf(stdout, " you have: ");
	...
	fprintf(stdout, " you have: ");
	ltmp         = append(ltmp, ltmp -> lead -> right, mknode(data[i]));
	displayf(ltmp, 0);
	fprintf(stdout, "should be: %s\n\n", output[num++]); 
	fflush(stdout);
	i            = i + 1;
