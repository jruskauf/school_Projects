...
int main()
{
    ...
    //
    i                                = qtmp -> data -> lead -> right -> VALUE;
    qtmp -> data -> lead -> right -> VALUE  = 29;
    fprintf(stdout,     "Test %d: Checking queues (1 of 2) ...\n", testno++);
    if (qtmp2 -> data -> lead -> right -> VALUE == i)
    ...
    if (qtmp -> data -> lead -> right -> VALUE == 29)
        fprintf(stdout, " you have: difference detected (SUCCESS)\n");
    else
        fprintf(stdout, " you have: ERROR state\n");
    ...
    if (qtmp2 -> front != qtmp2 -> data -> lead)
