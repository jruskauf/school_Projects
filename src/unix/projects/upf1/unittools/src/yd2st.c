...
#define  CONVERT         202.318
#define  DELIM           " <> "
#define  SRCUNITNAME     "yards"
#define  SRCUNIT         "yd"
#define  DSTUNITNAME     "stadions"
#define  DSTUNITNAMESING "stadion"
#define  DSTUNIT         "st"
...
int main(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "I:C:d:hnNp:qT:vV")) != -1)
    {
        switch (opt)
        {
            ...
            case 'T':
                if (strcasecmp(optarg, "egyptian") == 0)
                    convert  = 228.565;
                else if (strcasecmp(optarg, "itinerary") == 0)
                    convert  = 171.697;
                else if (strcasecmp(optarg, "olympic") == 0)
                    convert  = 192.476;
                else if (strcasecmp(optarg, "persian") == 0)
                    convert  = 214.348;
                else if (strcasecmp(optarg, "ptolemaic") == 0)
                    convert  = CONVERT;
                else
                {
...
void usage(char *argv)
{
    ...
    fprintf(stdout, "  -T \"TYPE\"   specify cubit type (default: \"ptolemaic\")\n");
    ...
    fprintf(stdout, "  egyptian    228.565%s\n", SRCUNIT);
    fprintf(stdout, "  itinerary   171.697%s\n", SRCUNIT);
    fprintf(stdout, "  olympic     192.476%s\n", SRCUNIT);
    fprintf(stdout, "  persian     214.348%s\n", SRCUNIT);
    fprintf(stdout, "  ptolemaic   %.3lf%s\n", CONVERT, SRCUNIT);
}
