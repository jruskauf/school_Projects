...
int main(int argc, char **argv)
{
	...
	//This handles testing abundant numbers to see whether they are semi-
	//perfect or weird.  This is done by starting from the largest factor
	//and working backwards, adding each factor to the sum of all the 
	//factors to the right of the "tracking" variable i've set.  This allows
	//for non-consecutive factors to be added together, increasing the like-
	//lihood of finding semi-perfect numbers.  While not perfect in and of 
	//itself, it's the simplest and best solution for the majority that i've
	//found so far. 
	if(sum > start){
	    int cstart = element-1;
	    int check = 0;
	    count = 0;
	    sum = 0;
	    while(cstart > 0 && check == 0){
		if(count < 0){
		    sum = sum + addnum[cstart];
		    cstart = cstart-1;
		    count = cstart;
		}
		if(sum + addnum[count] != start){
		    count--;
		} else {
		    printf(" (semi-perfect: %d + ", addnum[count]);
		    cstart++;
		    while(cstart != element-1){
			 printf("%d + ", addnum[cstart]);
			 cstart++;
		    }
		    printf("%d = %d)\n", addnum[cstart], start);
		    check = 1;
