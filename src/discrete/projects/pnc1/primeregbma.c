...
int main(int argc, char **argv)
{
	...
	while ((QTY != count || count == 0) && (number <= HighB || HighB == 0)){
		...
		//Here we check to see if the highest number is between the
		...
		if (plusnum >= bottomtrick && plusnum < toptrick){
			if ((minustally == trick && (QTY < count || count == 0)) && minnum >= LowB){
				fprintf(primelist, "%d ", minnum);
				QTY = QTY + 1;
			}
		
			if ((plustally == trick+1 && (QTY < count || count == 0)) && (plusnum <= HighB || HighB == 0)){	
