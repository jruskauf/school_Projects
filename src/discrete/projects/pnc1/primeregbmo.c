...
int main(int argc, char **argv)
{
	...
	while ((QTY != count || count == 0) && (number <= HighB || HighB == 0)){
		...
		int minnum = number - 1;
		//modcheck is added to be the housing variable for mod%2, as i
		...
		check1 = 0;
		check2 = 0;
		for (mod = 2; mod <= plusnum; mod = mod + 1){
			modcheck = mod%2;
			...
			if (mod <= minnum && check1 == 0){
				...
				//This checks if minremain is zero and if
				//modcheck is not 0.  If these two conditions
				//are met, check1 is set to 1, as to not run
				//the lower number loop.  This is also applied
				//to the higher number as well.
				} else if (modcheck != 0){
					check1 = 1;
				}
			}
		...
		if ((minustally == minnum && (QTY < count || count == 0)) && minnum >= LowB){
			fprintf(primelist, "%d ", minnum);
			QTY = QTY + 1;
		}
		
		if ((plustally == plusnum && (QTY < count || count == 0)) && (plusnum <= HighB || HighB == 0)){	
