...
//
// primreg.c : Calculate prime numbers using: brute force strategy
//
///////////////////////////////////////////////////////////////////////////
...
int main(int argc, char **argv)
{
	...
	int       num; //The value that is currently being tested
	...
	int primesize;
	unsigned long long int *primeray;
	...
	{
		HighB=0;
		primeray = malloc(sizeof(unsigned long long int) * 255);
		primesize = 255;
	}else{
		HighB=atoi(argv[4]);
		primeray = malloc(sizeof(unsigned long long int) * HighB);
		primesize = HighB;
	}
	...
	|* generated.                                                   *|
	'*==============================================================*/ 
	while ((QTY != count || count == 0) && (num <= HighB || HighB == 0)){
		...
		int tally = 1;
		for (mod = 2; mod <= number; mod = mod + 1){
