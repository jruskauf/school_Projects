...
int main(int argc, char **argv)
{
	...
	while ((QTY != count || count == 0) && (number <= HighB || HighB == 0)){
		trick = (topnum == number) ? trick + 1: trick;
		topnum = (topnum == number) ? trick * trick : topnum;
		//mod = (mod == 0) ? ((trick) * (trick)) : mod;
		
		remainder = number%mod;
		//printf("number: %d\nmod: %d\ntrick: %d\nremainder: %d\n", number, mod, trick, remainder);
		if(remainder == 0){	
			check = 1;
		}

		if((mod == trick && check == 0 ) || (number == 2 || number == 3)){
			fprintf(primelist, "%d ", number);
			QTY++;
			trick = (number == 2) ? trick+1 : trick;
			number++;
			mod = 2;
		} else if (mod >= trick || check == 1){
			number++;
			//trick = (number%trick+1 == 0) ? trick++ : trick;
			mod = 2;
			check = 0;
		} else {
			mod++;
